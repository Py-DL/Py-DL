...
from Py-DL import request, Stream


@mock.patch("Py-DL.streams.request")
...
@mock.patch(
    "Py-DL.request.head", MagicMock(return_value={"content-length": "6796391"})
)
@mock.patch(
    "Py-DL.request.stream",
    MagicMock(return_value=iter([str(random.getrandbits(8 * 1024))])),
)
def test_download(cipher_signature):
    with mock.patch("Py-DL.streams.open", mock.mock_open(), create=True):
        stream = cipher_signature.streams[0]
        stream.download()


@mock.patch(
    "Py-DL.request.head", MagicMock(return_value={"content-length": "16384"})
)
@mock.patch(
    "Py-DL.request.stream",
    MagicMock(return_value=iter([str(random.getrandbits(8 * 1024))])),
)
@mock.patch("Py-DL.streams.target_directory", MagicMock(return_value="/target"))
...
def test_segmented_stream_on_404(cipher_signature):
    ...
    with mock.patch('Py-DL.request.head') as mock_head:
        with mock.patch('Py-DL.request.urlopen') as mock_url_open:
...
def test_segmented_only_catches_404(cipher_signature):
    stream = cipher_signature.streams.filter(adaptive=True)[0]
    with mock.patch('Py-DL.request.stream') as mock_stream:
